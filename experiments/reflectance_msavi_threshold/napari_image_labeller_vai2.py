...
# Added ability to be ssh forwarded and opened for labelling using xpra
"""
...
1. On your local machine (laptop or desktop):
   ```bash
   ssh -Y your_username@remote_machine_ip
   ```

2. Once you're inside the remote machine (**after SSH**), you run:
   ```bash
   vglrun python your_script.py
   ```
   *(Or your Napari script with `os.environ["QT_OPENGL"] = "software"` if you prefer software rendering.)*
...
1. On your local machine:
   ```bash
   ssh your_username@remote_machine_ip
   ```

2. On the remote machine (**after SSH**):
   - Start a new Xpra session (this opens a *virtual* display `:100`):
     ```bash
     xpra start :100
     ```

3. Back on your local machine:
   - You open an **Xpra client** connection:
     ```bash
     xpra attach ssh:your_username@remote_machine_ip:100
     ```

4. On the remote machine (still inside SSH session):
   - Set the environment:
     ```bash
     export DISPLAY=:100
     ```
...
---
> **Summary:**  
> **Yes**, you always **SSH first**, and then **run these commands on the remote** machine.

---

# 💬 Quick diagram for intuition:

```
[YOUR LAPTOP] ---(ssh)---> [REMOTE MACHINE]
        |                       |
        | (X11 or xpra display)  | (python + napari)
```


This script labels the images of msavi and rgb of given land type. Change BASE_INPUT_DIR for other images.
---
"""
...
def load_required_images(land_path: Path) -> dict | None:
    ...
    for name, pattern in required_files.items():
        ...
        try:
            ...
            image = iio.imread(found_files[0])
            if name != "msavi":
                image = (image / 255).astype(np.uint8)
                image_normed = (image - image.min()) / (image.max() - image.min())
                images[name] = image_normed
            else:
                images[name] = image
...
class LabelingState:
    ...
    def setup_layers(self):
        ...
        if 'msavi' in self.images:
             self.msavi_layer = self.viewer.add_image(self.images['msavi'], name='MSAVI', colormap='RdYlGn', visible=False)
