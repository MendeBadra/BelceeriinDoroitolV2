"""This script is used for whole month data directory. Ex. 2025_07_Hustai_Drone"""
...
# quit()
output_root = "/media/razydave/CenAppMath/2025_07_Hustai_Drone_reflectance"
...
for category in os.listdir(input_root):
    ...
    if not os.path.isdir(category_path):
        continue
    for dji_dir in os.listdir(category_path):
        
        if dji_dir == "DJI_P4_MS":
            # print(f"dji_dir: {dji_dir}")
            dji_path = os.path.join(category_path, dji_dir)
            for plan_folder in os.listdir(dji_path):
                print(f"fplan folder = {plan_folder}")
                input_path = os.path.join(dji_path, plan_folder)
