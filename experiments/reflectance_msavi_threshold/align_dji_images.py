...
Output: 
    - blue to nir bands, all aligned using ECC. 5 blue_aligned
    - vegetation index TIF files, range of -1 to 1 ||  2 sh index
    - vegetation index plots. 2 sh plot
...
def main():
    ...
    image_names = (image_path).glob('DJI_*.TIF')
    ...
    for index, image in enumerate(image_names):
        print(f"{band_names[index] if index < len(band_names) else 'Unknown'} [{index}]: {image}")

    # Check for missing bands
    if len(image_names) < 5:
        print(f"Error: Expected 5 band images, found {len(image_names)}. Aborting.")
        sys.exit(1)
    ...
    fig = plot_index(ndvi, index='NDVI', cmap='viridis',domain=f" {image_path.parent.name}")
    # Plot MSAVI
    fig_msavi = plot_index(msavi, index='MSAVI', cmap='viridis',domain=f" {image_path.parent.name}")
    ...
    for index, (band_name, band) in enumerate(zip(band_names, [blue_aligned, green_aligned, red_edge_aligned, nir_aligned])):
